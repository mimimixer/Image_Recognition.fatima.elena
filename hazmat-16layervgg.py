...
# Parameters
path = "hazmat_categories_cropped_boxes_10000"
labelFile = "hazmat_categories_cropped_boxes.csv"
batch_size_val = 32
...
def create_model():
    ...
    number_of_nodes = 4096 # 500
        ...
        #model = VGG16(weights='imagenet', include_top=False, input_shape=(150, 150, 1)) # Sequential()
    model = Sequential()
    '''
    ...
    model.add(Conv2D(filters=512, kernel_size=(3, 3), padding="same", activation="relu"))
    model.add(Conv2D(filters=512, kernel_size=(3, 3), padding="same", activation="relu"))
    model.add(Conv2D(filters=512, kernel_size=(3, 3), padding="same", activation="relu", name="output_layer"))
    model.add(MaxPooling2D(pool_size=(2, 2), strides=(2, 2)))
...
# Save the model
model.save('hazmat_model_trained-16layer2.h5')
cv2.waitKey(0)
