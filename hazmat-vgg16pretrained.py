...
#os.environ['TF_USE_CUDNN_CONV_ALGO_FWD'] = '1'
...
def create_model():
    ...
    size_of_pool = (2,2)
    number_of_nodes = 4096 # 500
    optimizer = Adam(learning_rate=0.001)
    loss = "categorical_crossentropy"
    metrics = ["accuracy"]

    base_model = VGG16(weights='imagenet', include_top=False, input_shape=(image_Dimension[0], image_Dimension[1], 3)) # Sequential()
    model = Sequential()
    model.add(base_model)
    model.add(GlobalAveragePooling2D())
    model.add(Dense(1024, activation='relu'))
    model.add(Dense(number_of_classes, activation='softmax'))  # Assuming you have 10 classes

    '''
    model.add((Conv2D(number_of_filters, size_of_filters, input_shape=(image_Dimension[0], image_Dimension[1], 1), activation="relu")))
    ...
    model.add(Conv2D(filters=64, kernel_size=(5, 5), padding="same", activation="relu", input_shape=(image_Dimension[0], image_Dimension[1], 3)))
...
# Save the model
#model.save('hazmat_model_trained_vgg16.h5')
model.save('hazmat_model_trained-vgg16')
#model.save('hazmat_model_trained_vgg16', save_format='h5')

#cv2.waitKey(0)


# Load and preprocess the input image
img_path = 'radioactive-sign.png'  # Replace with the path to your image
...
# Generate the heatmap by averaging the weighted feature maps along the depth dimension
...
superimposed_img = cv2.addWeighted(cv2.cvtColor(np.uint8(255 * img_array[0]), cv2.COLOR_RGB2BGR), 0.6, heatmap, 0.4, 0)
